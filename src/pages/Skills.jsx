...
const LogoCPP=()=>(<svg viewBox="0 0 60 60"><circle cx="30" cy="30" r="28" fill="#004482"/><text x="9" y="38" fontSize="21" fontWeight="700" fontFamily="monospace" fill="white">C++</text></svg>)
const LogoJava=()=>(<svg viewBox="0 0 48 48"><path fill="#1565C0" d="M18 34s-2 1.2 1.4 1.6c4.2.4 6.3.4 10.9-.4 0 0 1.2.8 2.9 1.4-10.3 4.4-23.3-.3-15.2-2.6zM16.8 29s-2.2 1.6 1.2 2c4.4.4 7.8.4 13.8-.6 0 0 .8.8 2.1 1.3C22.1 35.5 9.3 32.5 16.8 29z"/><path fill="#1976D2" d="M28.4 21.3c2.5 2.9-.6 5.5-.6 5.5s6.3-3.2 3.4-7.2c-2.7-3.7-4.8-5.5 6.5-11.8 0 0-17.7 4.4-9.3 13.5z"/><path fill="#1565C0" d="M38.8 36.9s1.6 1.3-1.8 2.3c-6.4 1.9-26.6 2.5-32.2.1-2-.9 1.8-2.1 3-2.4 1.2-.3 1.9-.2 1.9-.2-2.2-1.6-14.4 3.1-6.2 4.4C23.7 44.5 45 41.1 38.8 36.9zM19.3 24.4S10 26.6 15.7 27.3c2.3.3 6.9.2 11.2-.1 3.5-.3 7-.9 7-.9s-1.2.5-2.1 1.1c-8.6 2.3-25.2 1.2-20.4-1.1 4.1-1.9 7.9-1.9 7.9-1.9zM27 0s5 5-4.7 12.7c-7.8 6.1-1.8 9.6 0 13.5-4.5-4-7.8-7.6-5.6-10.9C19.8 10.7 29.2 8.2 27 0z"/></svg>)
...
export default function Skills() {
  return (
    <main className="page">
      <div className="banner"><div className="wrap"><span className="tag">Technical Arsenal</span><div className="lg" style={{marginTop:18}}>Skills & Expertise</div><p style={{color:'var(--g3)',marginTop:12,fontSize:'0.95rem'}}>A full-stack data toolkit — from raw Python to polished Power BI dashboards.</p></div></div>
      <section className="sec">
        <div className="wrap">
          <span className="tag">Proficiency</span>
          <h2 className="md" style={{marginTop:14}}>Technical Skills</h2>
          <div className="skill-groups">
            {GROUPS.map(g=>(
              <div key={g.label}>
                <div className="sg-head"><div className="icon"><g.Logo/></div><span className="name">{g.label}</span></div>
                <div className="bars-grid">
                  {g.skills.map(s=>(<div className="bar-item" key={s.n}><div className="bar-hd"><span className="n">{s.n}</span><span className="p">{s.p}%</span></div><div className="bar-track"><div className="bar-fill" style={{width:`${s.p}%`}}/></div></div>))}
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="sec sec-alt">
        <div className="wrap">
          ...
          <h2 className="md" style={{marginTop:14}}>Tools & Platforms</h2>
          <div className="tools-grid">
            {TOOLS.map(t=>(<div className="tool-tile" key={t.n}><div className="t-logo"><t.L/></div><div className="t-name">{t.n}</div><div className="t-cat">{t.c}</div></div>))}
          </div>
        </div>
      </section>
      <div className="divider"/>
      <section className="sec">
        <div className="wrap">
          ...
          <div className="cert-grid">
            {CERTS.map(c=>(<div className="cert-item" key={c.n}><div className="cert-icon"><AwardIcon/></div><div className="cert-info"><h4>{c.n}</h4><p>{c.i}</p><span className="cert-date">{c.d}</span></div></div>))}
          </div>
        </div>
      </section>
    </main>
